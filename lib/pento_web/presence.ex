defmodule PentoWeb.Presence do
  ...
  @user_product_activity_topic "user_product_activity"
  @user_survey_activity_topic "user_survey_activity"

  def track_user_survey(pid, user_email) do
    Presence.track(
      pid,
      @user_survey_activity_topic,
      "SurveyUsers",
  ...
  def list_products_and_users do
    Presence.list(@user_product_activity_topic)
    |> Enum.map(&extract_product_with_users/1)
  end

  def list_surveys_user do
    Presence.list(@user_survey_activity_topic)
    |> Enum.map(&extract_product_with_users/1)
  end
